# 第六章 数理统计基础 整理笔记
本章是数理统计的入门基础，核心建立“总体-样本-统计量-抽样分布”的逻辑链条，为后续参数估计、假设检验提供理论与工具支撑。
...
### 一、频率直方图
#### 1. 作用
直观展示数值型数据的分布形态，当样本量足够大时，其外轮廓可近似代替总体的概率密度曲线。
...
4. 绘图：以小区间为底边，以\(\frac{f_i}{n}/\Delta\)为高绘制矩形，即频率直方图
...
| 对比维度 | 直方图 | 条形图 |
|---------|--------|--------|
| 含义 | 面积表示频数/频率，高度、宽度均有统计意义 | 长度表示频数，宽度固定无实际意义 |
| 排列方式 | 矩形连续排列（分组数据具有连续性） | 条形相互分开 |
| 适用数据 | 数值型连续数据 | 分类离散数据 |
...
#### 2. 箱线图的构成与绘制
- 基于5个特征值：最小值Min、第一四分位数\(Q_1\)、中位数\(M\)、第三四分位数\(Q_3\)、最大值Max
- 绘制方法
  1. 绘制水平数轴，标注5个特征值的位置
